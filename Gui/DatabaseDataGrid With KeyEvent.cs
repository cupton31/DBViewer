...
namespace DBViewer.Gui
{
	...
	public class DatabaseDataGrid : DataGrid
	{
		...
		private void TextBoxKeyDownEventHandler(object sender, KeyEventArgs e)
		{
			...
			if((e.KeyCode == Keys.S) && e.Control)
			{
				...
				DataGridTextBoxColumn textBoxColumn = (DataGridTextBoxColumn) this.TableStyles[0].GridColumnStyles[this.CurrentCell.ColumnNumber];
				// saves the DataGridCell's text in data soruce.
				table.Rows[this.CurrentCell.RowNumber][this.CurrentCell.ColumnNumber] = textBoxColumn.TextBox.Text;	
				// activates the DataGrid's OnKeyDown Event Handler.
				this.OnKeyDown(e);	
			}
		}
		/// <summary>
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			if((e.KeyCode == Keys.C) && e.Control)
			{
				...
				DataRow row = ((DataTable)this.DataSource).Rows[selectedDataGridRow];
		...
		private void PrepareForSave()
		{
			...
			{
				DataGridTextBoxColumn textBoxColumn = this.TableStyles[0].GridColumnStyles[i] as DataGridTextBoxColumn;
		...
		public void SaveDatabaseTable()
		{
			...
			string[] databaseInfo = this.Parent.Text.Split(new char[]{'.'});
	...
	public class DataGridImageColumn : DataGridColumnStyle
	{
		...
		//		/// <returns>Image from the file</returns>
		//private Image ReadImage(string file)
		//{
		//	try
		//	{
		//		//reads the bitmap from file.
		//		FileStream bmp = new FileStream(DBViewerConstants.ApplicationPath+"\\"+file, FileMode.Open, FileAccess.Read);
		//		BinaryReader reader = new BinaryReader(bmp);
		//		long streamLength = reader.BaseStream.Length;
		//		byte[] img = new byte[streamLength];
		//		// reads the image.
		//		int i = 0;
